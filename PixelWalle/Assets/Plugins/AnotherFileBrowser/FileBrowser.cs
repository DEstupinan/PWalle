...
namespace AnotherFileBrowser.Windows
{
  ...
  public void SaveFileBrowser(BrowserProperties browserProperties, string defaultFileName, Action<string> filepath)
        {
            var sfd = new VistaSaveFileDialog();
            sfd.Title = browserProperties.title ?? "Save File";
            sfd.InitialDirectory = browserProperties.initialDir ?? @"C:\";
            sfd.Filter = browserProperties.filter ?? "All files (*.*)|*.*";
