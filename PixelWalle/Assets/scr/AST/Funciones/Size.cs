...
public class Size : Function
{
    ...
    public override bool Check(CodeLocation location, List<Error> err, List<Expresion> arguments)
    {
        ...
        if (arguments[0].Type != ExpressionType.Number || (int)arguments[0].Value < 1)
        {
            err.Add(new Error(arguments[0].Location, ErrorType.Invalid, "Argument 1 must be numerical expresion greater than 1"));
            return false;
        }
